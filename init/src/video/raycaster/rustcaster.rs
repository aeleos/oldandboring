use boringos_std::math::{cosf32, sinf32};

pub const TWO_PI: f64 = 2.0 * boringos_std::math::PI64;
...
impl Engine {
    ...
    pub fn render(
        ...
        for x in 0..(self.projection_width as usize) {
            ...
            let wall_texture_x: u32 = if intersection.cell_side == 0 {
                (((intersection.y - (intersection.cell_y as f64 * cell_width)) % cell_width)
                    * (wall_texture.width - 1) as f64)
                    .round() as u32
            } else {
                (((intersection.x - (intersection.cell_x as f64 * cell_width)) % cell_width)
                    * (wall_texture.width - 1) as f64)
                    .round() as u32
            };
            ...
            let line_height: i32 =
                ((cell_height * self.projection_distance) / intersection_distance).round() as i32;
            ...
            for y in 0..(self.projection_height as usize) {
                ...
                if (y as i32) < line_screen_start {
                    ...
                    let ceiling_straight_distance =
                        (player_height / ceiling_row as f64) * self.projection_distance;
                    let angle_beta_radians = rotation - ray_angle;

                    let floor_actual_distance =
                        ceiling_straight_distance / angle_beta_radians.cos();

                    let mut ceiling_hit_x: f64 =
                        origin_x - (floor_actual_distance * ray_angle.cos());
                    let mut ceiling_hit_y: f64 =
                        origin_y - (floor_actual_distance * ray_angle.sin());

                    ceiling_hit_x -= ceiling_hit_x.floor();
                    ceiling_hit_y -= ceiling_hit_y.floor();

                    let texture_x: u32 =
                        f64::floor(ceiling_hit_x * (ceiling_texture.width - 1) as f64) as u32;
                    let texture_y: u32 =
                        f64::floor(ceiling_hit_y * (ceiling_texture.height - 1) as f64) as u32;
                    ...
                    let texture_y: u32 = f64::floor(
                        (line_y as f64 / line_height as f64) * (wall_texture.height - 1) as f64,
                    ) as u32;
                    let pixel = wall_texture.get_pixel(wall_texture_x, texture_y);

                    render_buffer[offset] = pixel.a;
                    render_buffer[offset + 1] = if intersection.cell_side == 1 {
                        pixel.b
                    } else {
                        pixel.g / 2
                    };
                    render_buffer[offset + 2] = if intersection.cell_side == 1 {
                        pixel.g
                    } else {
                        pixel.b / 2
                    };
                    render_buffer[offset + 3] = if intersection.cell_side == 1 {
                        pixel.r
                    } else {
                        pixel.r / 2
                    };
                    ...
                    let floor_straight_distance =
                        (player_height / floor_row as f64) * self.projection_distance;
                    let angle_beta_radians = rotation - ray_angle;

                    let floor_actual_distance =
                        floor_straight_distance / f64::cos(angle_beta_radians);

                    let mut floor_hit_x: f64 =
                        origin_x + (floor_actual_distance * f64::cos(ray_angle));
                    let mut floor_hit_y: f64 =
                        origin_y + (floor_actual_distance * f64::sin(ray_angle));

                    floor_hit_x -= floor_hit_x.floor();
                    floor_hit_y -= floor_hit_y.floor();

                    let texture_x: u32 =
                        f64::floor(floor_hit_x * (floor_texture.width - 1) as f64) as u32;
                    let texture_y: u32 =
                        f64::floor(floor_hit_y * (floor_texture.height - 1) as f64) as u32;
        ...
        for sprite in map.sprites.iter() {
            ...
            let sprite_distance: f64 =
                f64::sqrt(distance_x.powi(2) + distance_y.powi(2)) * f64::cos(rotation - theta);
            ...
            let sprite_screen_x: i32 =
                f64::round((self.projection_width as f64 / 2.0) + sprite_pixel_offset) as i32;

            let sprite_height: i32 =
                (f64::round(self.projection_distance / sprite_distance) as i32).wrapping_abs();
            let sprite_width: i32 =
                (f64::round(self.projection_distance / sprite_distance) as i32).wrapping_abs();
            ...
            let sprite_screen_start_y: i32 =
                -(sprite_height / 2) + (self.projection_height as i32 / 2);
            let sprite_screen_end_y: i32 =
                (sprite_height / 2) + (self.projection_height as i32 / 2);
            ...
            for sprite_screen_row in sprite_screen_start_x..sprite_screen_end_x {
                ...
                for sprite_screen_col in sprite_screen_start_y..sprite_screen_end_y {
                    if (sprite_screen_col < 0)
                        || (sprite_screen_col >= self.projection_height as i32)
                    {
                        continue;
                    }
                    ...
                    let texture_x: u32 = f64::round(
                        (sprite_row as f64 / sprite_width as f64) * (texture.width - 1) as f64,
                    ) as u32;
                    let texture_y: u32 = f64::round(
                        (sprite_col as f64 / sprite_height as f64) * (texture.height - 1) as f64,
                    ) as u32;

                    let offset =
                        ((sprite_screen_col * pitch as i32) + (sprite_screen_row * 4)) as usize;
    ...
    fn cast_ray(&self, origin_x: f64, origin_y: f64, angle: f64, map: &Map) -> RayIntersection {
        ...
        let mut ray_position_x: f64 = if is_ray_right {
            f64::ceil(origin_x)
        } else {
            f64::floor(origin_x)
        };
        ...
        while (ray_position_x >= 0.0) && (ray_position_x < map.width as f64)
            && (ray_position_y >= 0.0) && (ray_position_y < map.height as f64)
        {
            let tile_map_x: u32 =
            ...
            && (ray_position_y >= 0.0) && (ray_position_y < map.height as f64)
        {
            ...
            let tile_map_y: u32 =
                f64::floor(ray_position_y + (if is_ray_up { -cell_size } else { 0.0 })) as u32;
            ...
            if let Some(cell) = map.get_cell(tile_map_x, tile_map_y) {
                ...
                if (intersection_distance == 0.0)
                    || (x_intersection_distance < intersection_distance)
                {
